...
def categories():
    return "broadcast"
def shortDesc():
    return "(外部源直播)进入频道，三个主播连麦360P800K,限制下行500K20%丢包，统计设备B侧的卡顿率和延时"
def detailDesc():
    return "(直播)A B C设备一直在同一个频道内，设置设备的上行丢包策略，通过log计算播放侧的卡顿率"
...
def run():
    ...
    lib.ExeCmdCallBack(2, "setParameters,{\"che.video.LogcatVideoQoS\":1}")
    ...
    #lib.ExeCmdCallBack(1, "switchCamera")
    ...
    lib.ExeCmdCallBack(2, "setVideoEncoderConfiguration,640,360,15,800,0")
    lib.ExeCmdCallBack(2, "enableVideo")
    #lib.ExeCmdCallBack(2, "switchCamera")
    lib.ExeCmdCallBack(2, "setupLocalVideo,2,-1")
    lib.ExeCmdCallBack(2, "setupRemoteVideo,1,2,-1")
    lib.ExeCmdCallBack(2, "setupRemoteVideo,2,2,-1")
    lib.ExeCmdCallBack(2, "joinChannelByKey,nil," + Testchannelname + ",nil,3")

    time.sleep(10)
    lib.ExeCmdCallBack(1, "impairNet,0 0 0 500 20")
